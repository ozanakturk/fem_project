...
xdmf = io.XDMFFile(domain.comm, "heat_paraview/improved_euler.xdmf", "w")
...
k1 = ufl.TrialFunction(Vbig.sub(0))
k2 = ufl.TrialFunction(Vbig.sub(1))
v1 = ufl.TestFunction(Vbig.sub(0))
v2 = ufl.TestFunction(Vbig.sub(1))
kh = fem.Function(Vbig, name = "kh") 

P1 = k1 * v1 * ufl.dx - f.sub(0) * v1 * ufl.dx + ufl.dot(ufl.grad(u_n), ufl.grad(v1)) * ufl.dx \
    - (k2 * v2 * ufl.dx + dt * ufl.dot(ufl.grad(k1), ufl.grad(v2)) * ufl.dx \
    - f.sub(1) * v2 * ufl.dx + ufl.dot(ufl.grad(u_n), ufl.grad(v2)) * ufl.dx)
...
for n in range(num_steps):
    ...
    #################### First stage ####################
    a = fem.form(ufl.lhs(P1))
    L = fem.form(ufl.rhs(P1))
    ...
    solver.solve(b, kh.sub(0).x.petsc_vec)
    kh.sub(0).x.scatter_forward()
    """
    #################### Second stage ####################
    a = fem.form(ufl.lhs(P2))
    L = fem.form(ufl.rhs(P2))
    ...
    solver.solve(b, kh.sub(1).x.petsc_vec)
    kh.sub(1).x.scatter_forward()
    
    ###Updateing the previous step

    u_n.x.array[:] += 0.5 * dt * (kh.sub(0).x.array + kh.sub(1).x.array)
